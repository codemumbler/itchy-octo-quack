...
mainApp.controller('MainAppController', ['playerModel', 'aiService', 'gridService', 'notificationService', function (playerModel, aiService, gridService, notificationService) {
  ...
  this.select = function (x, y) {
    ...
    if (prevSelected) {
      ...
      if (prevSelected.hasClass('ship ' + currentPlayer.name) && !(selected.data('x') == prevSelectedCell[0] && selected.data('y') == prevSelectedCell[1]) && selected.find('.ship.' + currentPlayer.name).data('id') != prevSelected.data('id')) {
        if (playerModel.moveShip(prevSelected, selected))
          return;
      }
      if (prevSelected.hasClass('planet ' + currentPlayer.name + ' selected') && (selected.data('x') == prevSelectedCell[0] && selected.data('y') == prevSelectedCell[1])) {
